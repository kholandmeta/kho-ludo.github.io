...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "LudoClient": () => (/* binding */ LudoClient)
/* harmony export */ });
...
var LudoClient = /** @class */ (function () {
    function LudoClient() {
        ...
        console.log("LudoClient-In");
        var params = window.location.search.split('?')[1];
        var urlSearchParams = new URLSearchParams(params);
        var token = urlSearchParams.get('token');
        console.log('Token used: ', token);
        // this.sock = io("http://localhost:8888/ludo", {
        this.sock = (0,socket_io_client__WEBPACK_IMPORTED_MODULE_0__.io)("https://young-stream-43548.herokuapp.com/ludo", {
        ...
        this.initClick();
        var scope = this;
        // document.querySelector(".playerName").addEventListener("keypress", (e:KeyboardEvent) => {
        //     if (e.keyCode == 13) {
        //         e.preventDefault();
        //         scope.enterGame();
        //     }
    ...
    // enterGame() {
    //     let name = (document.querySelector(".playerName input") as any).value.toUpperCase();
    //     document.querySelector("#startGameDialogue").classList.remove("hidden");
    //     document.querySelector(".playerName").classList.add("hidden");
    //     this.sock.emit("playerName", name);
    // }
    LudoClient.prototype.initClick = function () {
        var _this = this;
        var sock = this.sock;
        var scope = this;
        document.addEventListener("click", function (e) { return __awaiter(_this, void 0, void 0, function () {
            var target, gottiId, src, ch, ids, i;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        target = e.target;
                        gottiId = target.id;
                        console.log("clicked", target, gottiId);
                        if (!gottiId.includes("playAgain")) return [3 /*break*/, 1];
                        document.querySelector("#endGameDialogue div").innerHTML = '';
                        document.querySelector("#endGameDialogue").classList.add("hidden");
                        document.querySelector("#startGameDialogue").classList.remove("hidden");
                        return [3 /*break*/, 8];
                    case 1:
                        if (!gottiId.includes("players")) return [3 /*break*/, 2];
                        console.log("got players");
                        sock.emit("joinGame", gottiId);
                        return [3 /*break*/, 8];
                    case 2:
                        if (!(target.className == "roll" || target.className.includes("gif"))) return [3 /*break*/, 3];
                        console.log("roll please");
                        sock.emit("roll", "hey");
                        return [3 /*break*/, 8];
                    case 3:
                        if (!gottiId.includes("sendMessage")) return [3 /*break*/, 4];
                        console.log("sendiong messahe");
                        scope.sendMessage(null);
                        return [3 /*break*/, 8];
                    case 4:
                        if (!gottiId.includes("gif")) return [3 /*break*/, 5];
                        src = gottiId.split(" ")[1];
                        src = "./images/GIFS/" + src + ".gif";
                        scope.sendMessage(src);
                        return [3 /*break*/, 8];
                    case 5:
                        if (!/^\d*$/.test(gottiId)) return [3 /*break*/, 6];
                        try {
                            ch = document.getElementById(gottiId).getElementsByClassName("Gotti");
                            if (ch[0]) {
                                ...
                                console.log("Sending Gott clicked event with ids", ids);
                        ...
                        return [3 /*break*/, 8];
                    case 6:
                        console.log("Sendung Gott clicked event with id", gottiId);
                        return [4 /*yield*/, sock.emit("gottiClicked", gottiId)];
                    case 7:
                        _a.sent();
                        _a.label = 8;
    ...
    LudoClient.prototype.init = function () {
        var _this = this;
        var sock = this.sock;
        var scope = this;
        ...
        //     document.querySelector(".playerName").classList.add("hidden");
        // })
        scope.sock.on("connect_error", function (err) {
        ...
        scope.sock.on("connecting", function () {
            console.log(scope.sock.id);
        });
        sock.on("waitForPlayers", function (num) {
        ...
        sock.on("startGame", function (availablePlayers, gottisInside, playerIds, names) {
            document.querySelector("#startGameDialogue").classList.add("hidden");
            document.querySelector(".waitingForPlayers").classList.add("hidden");
            scope.GAMEDATA.playerIds = playerIds;
            ...
            for (var i = 0; i <= availablePlayers.length; i++) {
                if (availablePlayers.includes(i)) {
                    ...
                    document.querySelector("." + CONSTANTS.defaultColors[i] + ".home").appendChild(name_1);
                    //placing gottis in positions
                    for (var j = 0; j < 4; j++) {
                        var gotti = document.createElement("img");
                        name_1.classList.add("name");
                        gotti.classList.add("Gotti");
                        gotti.id = gottisInside[i][j];
                        var col = gotti.id.slice(0, gotti.id.length - 1);
                        gotti.src = './images/gottis/' + col + '.png ';
                        var pnt = document.querySelectorAll(".home_" + col + ".inner_space");
                        pnt[j].appendChild(gotti);
                    }
                }
            }
        });
        sock.on("showMessage", function (message, color) {
            scope.showMessage(message, color);
        });
        sock.on("powerUpTime", function () { return __awaiter(_this, void 0, void 0, function () {
            var pp;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        pp = document.querySelector(".powerUps");
        ...
        sock.on("gameOver", function (winners) {
        ...
        sock.on("playerIndicator", function (currentPlayerColor, id) {
            console.log("adding highlight");
            var all = document.querySelectorAll(".home .profilePic");
            ...
            scope.GAMEDATA.currentPlayerColor = currentPlayerColor;
            var home = document.querySelector("." + currentPlayerColor + ".home .profilePic");
            home.classList.add('highLight');
            if (sock.id === id) {
                document.querySelector(".gif").classList.add("heartBeat");
            }
        });
        sock.on("removeGottiShake", function () {
            document.querySelector(".gif").classList.remove("heartBeat");
        });
        sock.on("removePlayer", function (color) {
            console.log("removing player");
            var name = document.querySelector("." + color + " .name");
            name.parentElement.removeChild(name);
            var profile = document.querySelector("." + color + " .profilePic");
            profile.parentElement.removeChild(profile);
            for (var i = 1; i <= 4; i++) {
                var gotti = document.querySelector("#" + color + i);
                if (gotti) {
                    gotti.parentElement.removeChild(gotti);
                }
            }
        });
        sock.on("rollTheDice", function (movementAmount) { return __awaiter(_this, void 0, void 0, function () {
            var gif;
            return __generator(this, function (_a) {
        ...
        sock.on("removeShakeAnimation", function (gottisInside, gottisOutside) {
            scope.removeShakeAnimation(gottisInside, gottisOutside);
        });
        sock.on("moveGotti", function (id, playerIndex, positions, gottisInside, gottisOutside, result) { return __awaiter(_this, void 0, void 0, function () {
            var g, fd, i, fdGottis, fdGottis_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        scope.GAMEDATA.playerIndex = playerIndex;
                        scope.removeShakeAnimation(gottisInside, gottisOutside);
                        g = document.getElementById(id);
                        i = 0;
                        _a.label = 1;
                    case 1:
                        if (!(i < positions.length - 1)) return [3 /*break*/, 5];
                    ...
                    case 3:
                        if (i == positions.length - 1) {
                            if (result["killed"])
                                scope.killGotti(result['killed']);
                            if (result['powerUp'])
                                scope.addPowerUp(result['powerUp']);
                            if (result["gottiHome"])
                                scope.gottiHome(result['gottiHome']);
                    ...
                    case 5:
                        if (scope.GAMEDATA.playerIds[scope.GAMEDATA.playerIndex] == sock.id) {
        ...
        sock.on("getGottiOut", function (id, position, gottisInside, gottisOutside) {
            var removeShakeAnimation = function (gottisInside, gottisOutside) {
                for (var i = 0; i < gottisOutside.length; i++) {
                    for (var j = 0; j < gottisOutside[i].length; j++) {
                        var gotti = document.querySelector("#" + gottisOutside[i][j]);
                        if (gotti)
                            gotti.classList.remove("useMe");
                    }
                }
                for (var i = 0; i < gottisInside.length; i++) {
                    for (var j = 0; j < gottisInside[i].length; j++) {
                        var gotti = document.querySelector("#" + gottisInside[i][j]);
                        if (gotti)
                            gotti.classList.remove("useMe");
                    }
                }
            ...
            scope.removeShakeAnimation(gottisInside, gottisOutside);
            var fd = document.getElementById(position);
            var g = document.getElementById(id);
            fd.appendChild(g);
            //nikalda kheri position ma multiple gotti check
            var fdLen = fd.getElementsByClassName("Gotti");
            if (fdLen.length == 2) {
                fd.classList.add("twoGotti");
            }
        ...
        sock.on("addPowerUp", function (destinationID) {
            scope.addPowerUp(destinationID);
        });
        sock.on("killGotti", function (killed) {
            scope.killGotti(killed);
        });
        sock.on("removePowerUp", function (type) {
            var pp = document.querySelector(".powerUps");
            pp.classList.remove("timer");
            var p = document.querySelector(".powerUps");
            var c = p.querySelector("." + type);
            p.removeChild(c);
        });
        sock.on("addShakeAnimation", function (movableGottis) {
            movableGottis.forEach(function (element) {
                var d = document.getElementById(element);
                d.classList.add("useMe");
            });
        });
    };
    LudoClient.prototype.removeShakeAnimation = function (gottisInside, gottisOutside) {
        for (var i = 0; i < gottisOutside.length; i++) {
            for (var j = 0; j < gottisOutside[i].length; j++) {
                var gotti = document.querySelector("#" + gottisOutside[i][j]);
                if (gotti)
    ...
    LudoClient.prototype.showMessage = function (text, color) {
        var d = document.querySelector("." + color + ".home" + " .message");
        var p;
    ...
    LudoClient.prototype.sendMessage = function (src) {
        var message;
        if (src) {
            message = src;
        }
    ...
    LudoClient.prototype.addPowerUp = function (destinationID) {
        var dest = document.getElementById(destinationID);
        var child = dest.getElementsByClassName("powerUp")[0];
        dest.removeChild(child);
        if (this.GAMEDATA.playerIds[this.GAMEDATA.playerIndex] == this.sock.id)
            document.querySelector(".powerUps").appendChild(child);
    };
    LudoClient.prototype.killGotti = function (killed) {
        var color = killed.substr(0, killed.length - 1);
        var spots = document.getElementsByClassName("home_" + color);
    ...
    LudoClient.prototype.gottiHome = function (id) {
        var col = id.replace(/[0-9]/g, "");
        var gotti = document.querySelector('#' + id);
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "GameLobby": () => (/* binding */ GameLobby)
/* harmony export */ });
/* harmony import */ var socket_io_client__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! socket.io-client */ "./node_modules/socket.io-client/build/esm/index.js");
...
var GameLobby = /** @class */ (function () {
    ...
    GameLobby.prototype.init = function () {
        var token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJwYXlsb2FkIjp7ImlkIjoyNTU5LCJwdWJsaWNBZGRyZXNzIjoiMHgyZmUzMThkZjAxY2VmZjY0ZDBlZjcyMDdmMzc2NDljYjVhNzdmOWNkIn0sImlhdCI6MTY0Nzk3NDcyMX0.MOFt357TX5isj3hfMLplb5RT4MXE1o6G1hADKpMc-yo';
        var socket = (0,socket_io_client__WEBPACK_IMPORTED_MODULE_0__.io)("https://young-stream-43548.herokuapp.com/game-lobby", {
            auth: { token: token }
        }).connect();
        console.log("....");
...
/*!**********************!*\
  ...
  \**********************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _lobby__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./lobby */ "./src/lobby.ts");
